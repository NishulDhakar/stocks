      ...
      const profiles = await Promise.all(
        top.map(async (sym) => {
          try {
           ...
           const profile: Profile = await fetchJSON<Profile>(url, 3600);
            return { sym, profile } as { sym: string; profile: Profile};
          } catch (e) {
            console.error('Error fetching profile2 for', sym, e);
            return { sym, profile: null } as { sym: string; profile: Profile | null};
          }
        })
      );
...
const name: string | undefined = profile?.name || profile?.ticker || undefined;
          ...
          (r as { __exchange?: string }).__exchange = exchange; // internal only
          return r;
        })
